...
export interface LoanParams {
  ...
  /** Último CDI/Selic % a.m. (BCB) para competências ainda sem publicação oficial. */
...
export function calculateLoan(params: LoanParams): LoanRow[] {
  ...
  const idxUltimaCarencia =
    gracePeriodMonths > 0 ? schedule.findIndex((r) => r.isGrace && r.month === gracePeriodMonths) : -1;
  ...
  for (let i = 0; i < schedule.length; i++) {
    if (cpcMode === 'fiscal' && schedule[i].isGrace) {
       ...
       */
      if (schedule[i].month < gracePeriodMonths) {
