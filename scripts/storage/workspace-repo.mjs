...
/**
 * Copia workspace de um token para outro (se o destino estiver vazio).
 */
export async function cloneOfficeWorkspace(sourceToken, targetToken, uid = '') {
  const src = String(sourceToken || '').trim();
  const tgt = String(targetToken || '').trim();
  if (!src || !tgt || src === tgt) {
    return { ok: false, skipped: true, reason: 'invalid_tokens' };
  }

  await ensureOfficeRow(tgt, uid);
  const srcOffice = await getOffice(src);
  if (!srcOffice) {
    return { ok: false, skipped: true, reason: 'source_empty' };
  }

  const tgtOffice = await getOffice(tgt);
  const tgtHasCompanies = asArray(tgtOffice?.companies_registry).length > 0;
  let copiedOffice = false;
  ...
  if (!tgtHasCompanies) {
    await setOffice(
      ...
      {
        ...srcOffice,
        name: String(tgtOffice?.name || srcOffice.name || tgt),
      },
      uid,
    );
    copiedOffice = true;
  ...
  const tgtManagers = await listManagerByOffice(tgt);
  if (tgtManagers.length === 0) {
    const srcManagers = await listManagerByOffice(src);
    for (const m of srcManagers) {
      await setManager(tgt, m.company_slug, m, uid);
      copiedManagers += 1;
    }
  }

  const srcManagers = await listManagerByOffice(src);
  const slugs = [...new Set(srcManagers.map((m) => String(m.company_slug || '').trim()).filter(Boolean))];
  for (const slug of slugs) {
    const pastas = await listExtratoPastas(src, slug);
    const existing = await listExtratoPastas(tgt, slug);
    if (existing.length > 0) continue;
    for (const p of pastas) {
      let pdfBase64;
      if (p.pdfObjectKey) {
        const buf = await getExtratoPastaPdfBuffer(src, p.id);
        if (buf) pdfBase64 = buf.toString('base64');
      }
      await saveExtratoPasta(tgt, slug, {
        id: undefined,
...
/**
 * Copia sufixos gerenciais ausentes/vazios do token origem → destino
 * (ex.: plano salvo em INOV mas sessão ativa em CL-FN14-AZ4ZV81Y).
 */
export async function mergeMissingManagerSuffixes(sourceToken, targetToken, uid = '') {
  const src = String(sourceToken || '').trim();
  const tgt = String(targetToken || '').trim();
  if (!src || !tgt || src === tgt) {
    return { ok: false, skipped: true, reason: 'invalid_tokens', copiedSuffixes: 0 };
  }

  await ensureOfficeRow(tgt, uid);
  const srcManagers = await listManagerByOffice(src);
  const tgtManagers = await listManagerByOffice(tgt);
  const tgtBySlug = new Map(tgtManagers.map((m) => [String(m.company_slug || ''), m]));

  let copiedSuffixes = 0;
  for (const srcM of srcManagers) {
    const slug = String(srcM.company_slug || '').trim();
    if (!slug) continue;
    const tgtM = tgtBySlug.get(slug);
    const patch = {
      company_slug: slug,
      company_name: String(srcM.company_name || tgtM?.company_name || ''),
      data: {},
    };

    for (const [suffix, rows] of Object.entries(srcM.data || {})) {
      if (!suffix) continue;
      if (!Array.isArray(rows) || rows.length === 0) continue;
      const tgtRows = tgtM?.data?.[suffix];
      if (Array.isArray(tgtRows) && tgtRows.length > 0) continue;
      patch.data[suffix] = rows;
      copiedSuffixes += 1;
    }

    if (Object.keys(patch.data).length > 0) {
      await setManager(tgt, slug, patch, uid);
    }
  }

  return { ok: true, from: src, to: tgt, copiedSuffixes };
}
