...
function resolveS3EndpointUrl() {
  const full = String(process.env.MINIO_S3_ENDPOINT || process.env.S3_ENDPOINT || '').trim();
...
function getMinioClient() {
  if (!client) {
    ...
    client = new S3Client({
      region: String(process.env.MINIO_REGION || process.env.S3_REGION || 'us-east-1').trim(),
      endpoint: resolveS3EndpointUrl(),
      forcePathStyle: true,
      credentials: {
